...
//keep track of token ids
    ...
    // input to contract for .endingName
    string public tld;
    ...
    constructor(string memory _tld) payable ERC721("Polygon Name Service","PNS"){
        tld=_tld;
        console.log("Polygon Name Service Deployed",_tld);
    }

    function price(string calldata name) public pure returns(uint){
        uint len = StringUtils.strlen(name);
        require(len>0);
        // Shorter Are Expensive
        // len = 3 --> 0.5 MATIC
        // len = 4 --> 0.3 MATIC
        // len = 1 --> 0.1 MATIC 
        if(len == 3){
            return 5 * 10**17; //5* 10^18 MATIC 
        }else if(len ==4){
            return 3 * 10**17; 
        }
        else{
    ...
    //register function to add names to map
    ...
    // Domain owner address retireval
